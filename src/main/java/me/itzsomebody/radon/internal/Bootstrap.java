...
public class Bootstrap { // Eyyy bootstrap bill
     ...
     */
    private final Map<String, ClassNode> extraClasses = new HashMap<>();

    /**
     * Resources which "pass through" the obfuscator.
     */
    private final Map<String, byte[]> passThru = new HashMap<>();
    ...
    private void buildHierarchy(ClassNode classNode, ClassNode sub) {
        ...
        if (sub != null) {
            hierarchy.get(classNode.name).subClasses.add(sub.name);
        }
    }
    ...
    class CustomClassWriter extends ClassWriter {
        ...
        private boolean isAssignableFrom(ClassNode clazz1, ClassNode clazz2) {
            ...
            ClassTree firstTree = getTree(clazz1.name);
            ...
            while (!searchThese.isEmpty()) {
                ...
                if (children.add(s)) {
                    ClassTree tempTree = getTree(clazz2.name);
